...
## 2026-07-02: BT プロファイルが off のまま固着する不具合の修正

### 問題

`toggle-bt-profile.sh` の A2DP シンク検出待ちループが `off` のまま固着し, プロファイルが切り替わらないことがあった。

### 根本原因

A2DP シンク検出の待機ループが `2ch 48000Hz`（ステレオ・48kHz）の出現を条件にしていたが, コーデックネゴシエーションによっては A2DP シンクが `44100Hz` で現れることがあり, サンプルレート条件が満たされずポーリングが失敗していた。

### 修正内容

`pactl list sinks short` の出力に対する grep 条件を `2ch`（ステレオであること）のみに緩和し, サンプルレートを問わないようにした。

### 知見

- A2DP シンクのサンプルレートは常に 48000Hz とは限らない。コーデックネゴシエーションの結果 44100Hz になることがあり, サンプルレート固定を前提にした検出条件は壊れやすい。
