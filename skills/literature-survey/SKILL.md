...
| Phase 1 — RQ | Phase 1 の例示パターン(approaches / evaluation conditions / unresolved challenges)で 2-4 個を auto-derive, Methodology → Frame に記録 |
...
- **Depth / Breadth**: 採録規模を決定
  ...
  **target 内のどこに着地すべきか**: snowballing が自然飽和した点
  ...
  - 上限を超えたが RQ が複数クラスタを要求する場合: Map Checkpoint でユーザーに
    採録数を報告し承認を得れば超過を許容する. Methodology に理由と承認を記録
...
- **B. カテゴリ橋渡し性 + 影響力**: 以下の両方を満たす:
  ...
  - **citation 数が取得できない環境**(Semantic Scholar MCP / OpenAlex script
    ...
    WebSearch hit / 本 survey 内被引用頻度)で代替判定する. 判定基準は
    各軸 High / Mid / Low 評価のうち **≥2 軸 High で上位 1/3 相当**と認定(詳細は ref)
...
`/paper-summary` 内でも self-verification(Step 6: 原典照合検証)が走るが,
同一エージェントの自己検証には共有バイアスの限界がある. literature-survey では
**別エージェントによるクロス検証**を追加し, 精度を引き上げる.

**手順:**

1. 全ハブの `/paper-summary` が完了した後, ハブごとに**検証エージェント**を dispatch する
   (生成エージェントとは別のサブエージェント. 並列可)
2. 検証エージェントには以下を渡す:
   - ハブ論文の PDF(`./literature/papers/{citekey}/main.pdf`)
   - 生成された深読みノート(`./literature/papers/{citekey}/{slug}.md`)
3. 検証エージェントのタスク:
   - ノートの各セクションから事実主張を抽出
   - PDF の該当箇所と照合し, 不一致・捏造・文脈歪曲を検出
   - 検出結果を構造化して返す(主張 / PDF 該当箇所 / 判定: OK or 不一致の内容)
4. 不一致が検出された場合, 深読みノートを修正する
5. 修正件数と内容を Methodology → Verify に記録

**コスト制御**: 検証エージェントは **Sonnet tier** で dispatch し, PDF 全文精読ではなく
ノートの主張に対応するセクションのみを targeted に読む.

**Auto-execution mode**: 検証は省略しない(頓珍漢な出力を防ぐのが本ステップの目的).
ただし呼び出し側が"cross-verify skip"を明示した場合のみ省略可.
Methodology に"hub cross-verification skipped per caller instruction"と明記.
...
**2. Synthesis Trace-back(内容整合性)**

Phase 4 で生成した synthesis(thesis / foundation / progress / gap)の主要主張が,
ハブ深読みノートの記述と整合するかを確認する. 対象は Phase 3 で深読みノートが
生成されたハブ論文への言及に限る(非ハブ論文への言及は本ステップの対象外):

1. synthesis の各セクションから, ハブ論文を根拠とする事実主張を抽出
   (数値, 手法の特徴, 比較結果, 限界の記述など. 主張単位で分割)
2. 該当するハブ深読みノートの記述と照合
3. 深読みノートに根拠がない synthesis 主張 → 正しい記述をノートから復元
   できれば修正, 復元不可能であれば削除
4. 深読みノートの記述を歪曲した synthesis → 表現を修正

判定の目安: 否定的主張("X は未実施"等)はノートに該当記載がないことを根拠に
整合と判定する. ノートの記述と矛盾しない合理的意訳も整合とみなす.

Phase 3 の Hub Deep Read クロス検証が深読みノート自体の正確性を担保し,
本ステップは synthesis が深読みノートを正しく引用しているかを担保する.
2 段階で"原典 → 深読みノート → synthesis"の連鎖全体を検証する.
