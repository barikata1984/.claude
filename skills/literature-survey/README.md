...
| 日付 | 変更内容 |
|------|---------|
| 2026-05-02 | スキル設計ベストプラクティスに沿った大規模リファクタ。SKILL.md を 510 → 410 行に圧縮（公式 500 行ガイドライン clear）。日本語ライティング規約を `~/.claude/rules/japanese_style.md` に分離（CLAUDE.md 経由で他 skill も共有）。Map Checkpoint テンプレ・Hub citation proxy・並列 wikilink coordination を `references/` 配下に切り出して progressive disclosure 化。Auto-execution Mode を 1 つの表に集約。Phase 5 に Reference Processing と Final Quality Check を統合。Phase 3 で Hub 除外 → Hub 選定の順に整理。dead script `extract_sections.py` と歴史文書 `HANDOFF.md` を `.archive/` へ退避。 |
