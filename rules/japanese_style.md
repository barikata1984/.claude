...
チャット応答・生成文書とも、和文でも半角約物スタイルで書く。`.md` ファイルでは
約物・括弧の変換をフック (`format-ja-typography`) が自動で行うので、生成時は
特にダッシュとスペース（フックが扱わない項目）に注意する。

- **句読点・括弧は半角に半角スペースを添える**: 文末は `. `、節区切りは `, `、
  ほかに `! ` `? ` `: `。引用は `"" ''`、括弧は `()`。約物の後ろに半角スペースを
  置く（ただし行末・閉じ括弧の直前は詰める）。範囲は `〜`、中点は `・` のまま残す。
- **ダッシュ**: em dash（—）は使わない。補足・挿入は括弧・読点・コロンで表す。
  en dash（–）は数値範囲・開いた複合語にのみ使い、前後にスペースを置く挿入用法は禁止。
- **和欧境界スペース**: 和文中の英単語・数字の前後に半角スペースを入れる
  （例: `Transformer を使う`、`GPT-4 は`）。例外として詰める: 定着語 `Tシャツ`
  `Jリーグ`、数字+和文助数詞・年号 `3個` `2024年`、`%`・`°`（`50%`・`45°`）。
  数字+ラテン単位は空ける（`5 GHz`）。
- **半角統一**: 算用数字・ラテン文字は半角。全角英数は使わない。
- ASCII の `.` `,` 等は変換対象でないので、小数 `3.14`・略語 `e.g.`・URL・コードは
  そのまま保たれる。
