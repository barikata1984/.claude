...
本サーベイが明らかにする分野の根本的未解決問題は、**慣性パラメータ推定と安定配置計画の断絶**である。推定側では、1986年の線形回帰定式化以来、バッチ最小二乗（Category A）、オンライン再帰推定（Category B）、物理整合性保証（Category C）、協働ロボット適応・学習ベース手法（Category D）と4つの技術系譜が発展し、質量・CoM・慣性テンソルの推定は成熟した技術となっている。配置側では、接触ロバスト性分析（Nadeau & Kelly 2025）、物理誘導拡散モデル（Nadeau et al. 2025）、学習ベース汎化配置（AnyPlace, Placeit!）が安定配置の予測・計画に取り組んでいる。

しかし、55本の網羅的調査において、**F/Tセンサで推定した慣性パラメータを配置計画に入力として供給するエンドツーエンドパイプラインを実装・実証した研究は1本も発見されなかった**。最も近い研究は3つある。Nadeau et al. (2022) はF/Tベースの高速推定を実装し「計画への活用」に言及するが配置パイプラインは未実装。Nadeau & Kelly (2025) はCoMを使う安定配置計画を実現するがCoMは既知前提。Lerner et al. (2024) はF/Tフィードバックで配置を行うがCoMを推定せず幾何中心で近似し、著者自身がCoM未知を制約と認めている。

この断絶は、推定コミュニティと配置計画コミュニティの研究文化の違いに起因する。推定研究はセンサ精度・条件数・物理整合性といった「推定品質」を最適化するが、推定結果の下流タスクへの伝搬は範囲外とする。配置研究は幾何表現・拡散モデル・物理シミュレーションによる「配置品質」を追求するが、入力となる物理パラメータは既知として扱う。この二つの品質指標を統一するフレームワークの不在が、分野横断的な進展を阻んでいる。
...
4. **物理整合性制約**: Sousa & Cortesao (2014) のLMI、Traversaro et al. (2016) の多様体最適化、Janot & Wensing (2021) のSDP、RSS2025の区間算術が、推定値が物理的に実現可能な剛体に対応することを保証する。これは下流タスク（配置計画を含む）に推定値を供給する際の前提条件となる。
...
1. **End-to-End Estimation-to-Placement Pipeline**

55本の網羅的調査において、F/Tセンサベースの慣性パラメータ推定と安定配置計画を直接接続したエンドツーエンドシステムは発見されなかった。Category A-DとCategory Eの論文群は、それぞれ推定品質と配置品質を独立に最適化しているが、推定誤差が配置安定性に与える影響の定量的分析は皆無である。Nadeau & Kelly (2025) のプランナはCoMを入力として受け取る設計であり、推定値への置換はアーキテクチャ的に可能だが、推定誤差に対する感度分析は未実施である。このパイプラインが実現すれば、未知物体の「把持→推定→安定配置」がセンサフィードバックのみで完結するシステムが構築可能になる。
...
3. **Non-Uniform Density Handling**

Category Eの学習ベース配置手法（AnyPlace, Placeit!, SPOTS）は訓練データ生成においてシミュレーション上の物理特性ランダマイズを適用するが、密度の非均一性（幾何重心と質量重心の乖離）を体系的に扱った報告はない。Nadeau & Kelly (2025) は非均一密度を明示的に許容するが既知CoMを前提とする。Jin et al. (2025) のU-GRAPHはF/Tベースの3D CoM推定で非均一密度物体を扱うが配置への接続はない。Kang et al. (2025) は視覚ベースのCoG推定を把持に適用するが、触覚・力覚フィードバックとの融合や配置計画への拡張は未着手である。

4. **Uncertainty Propagation from Estimation to Planning**

Category Cの物理整合性手法は推定パラメータの実現可能性を保証するが、推定不確実性の定量化とその下流タスクへの伝搬は限定的である。RSS2025が区間算術による保守的パラメータ境界を計算し、Jin et al. (2025) がBayesian Neural Networkで不確実性を出力するが、これらの不確実性情報を配置安定性解析に伝搬して「推定誤差下でもロバストな配置」を保証する枠組みは存在しない。確率的配置計画（推定CoMの信頼区間から安定配置の確率を計算する等）は、推定と配置を橋渡しする自然な理論的枠組みとなりうる。
...
|------|---------|----------|
| 1: F/T-to-Placement Pipeline | F/T推定（Nadeau 2022）と配置計画（Nadeau & Kelly 2025）は独立に成熟しており、接続のみが未達成 | 既存手法を直結し、推定誤差→配置安定性の感度分析を実施 |
| 2: Uncertainty-Aware Robust Placement | CoM推定には必然的に不確実性があり、点推定値での配置計画は脆弱 | 推定不確実性を配置安定性解析に伝搬し、最悪ケースでも安定な配置を計画 |
...
#### Seed 1: End-to-End F/T-Estimated CoM to Stable Placement Pipeline

##### Seed 1 — Academic Contribution

Gap 1（推定と配置の断絶）を直接埋める研究。55本の調査で確認された通り、F/Tベースの慣性パラメータ推定を安定配置計画に供給するエンドツーエンドシステムは前例がない。Nadeau et al. (2022) の推定パイプラインとNadeau & Kelly (2025) の配置プランナは同一研究グループの成果であり、接続は技術的に自然だが未実装・未検証である。本Seedの学術的貢献は、(a) 統合パイプラインの実装と実証、(b) 推定誤差（CoM位置誤差、質量誤差）が配置安定性（接触ロバスト性メトリクス）に与える影響の定量的感度分析、の二点である。

##### Seed 1 — Required Components

1. F/Tセンサ搭載協働ロボットプラットフォーム
2. 把持後の慣性パラメータ推定モジュール（質量、CoM位置、オプションで慣性テンソル）
3. 推定CoMを入力とする安定配置プランナ
4. 非均一密度物体のテストセット（幾何重心と質量重心が乖離する物体群）
5. 推定誤差→配置安定性の感度分析フレームワーク
...
| 慣性パラメータ推定 | Available | Nadeau et al. (2022) PMD法が質量・CoMを1.5秒で推定。既知形状が必要。形状未知の場合はJin et al. (2025) U-GRAPHが代替候補 |
| 安定配置プランナ | Available | Nadeau & Kelly (2025) の接触ロバスト性プランナがCoMを入力として受け取る設計 |
| 非均一密度テストセット | Adaptable | 既存のYCBオブジェクトセットに内部重りを追加して構築可能。標準ベンチマークは存在しない |
| 感度分析フレームワーク | New development required | CoM推定誤差と配置ロバスト性メトリクスの関数関係を導出・検証する解析的/実験的枠組み |
...
- Papers checked: 55
- Passed (DOI/URL verified): 45
- DOI wrong → corrected: 2 (Yu2022, Swevers2002)
- Remaining unverified (arXiv-only, abstract confirmed): 8
