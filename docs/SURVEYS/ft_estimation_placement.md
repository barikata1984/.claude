...
配置手法は大きく学習ベースと物理ベースに分かれるが、両者は異なる形で同じ盲点を抱えている。**学習ベース手法**（AnyPlace, UOP-Net, Placeit!, 6-DoFusion）は、均一密度シミュレーションで生成された訓練データから安定配置を暗黙的に学習する。これらの手法はCoMを明示的に計算も入力もせず、安定性判定はネットワークの重みに埋め込まれている。したがって、推定CoMを後から注入する機構がそもそも存在しない。**物理ベース手法**（Nadeau & Kelly 2025, Nadeau et al. 2025 diffusion, SPOTS, Placeit!のシミュレーション層）は、既知のCoM・質量・形状を前提に解析的またはシミュレーションベースで安定性を評価するが、これらの入力が推定値に置き換わった場合のロバスト性を検証した研究は皆無である。
