...
5. **under-retrieval（外部知識導入のサボり）対策** — hallucination とは別軸（対・事実の
   情報取得衛生）として CLAUDE.md に「外部事実の検証」節を独立配置。強制ハーネスは原理的に
   不可（検索是非はモデル内部判断、Stop ゲートは撤回不可・over-response 誘発）と確認したため、
   薄い指示 + empirical 測定を主とし、遵守率が不足する場合のみ軽量 UserPromptSubmit 分類 hook を
   後付けする段階導入。常駐デーモンと hallucination 対策一式は不採用
